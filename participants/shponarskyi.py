...
def process_title_ratings(spark_session, title_ratings_path, title_basics_path, f):
    ...
    average_rating_df = ratings_df.join(title_basics_df, "tconst").filter("numVotes >= 1000").orderBy("averageRating", ascending=False).limit(100)

    average_rating_df.show()

    # Як змінювалась середня оцінка фільмів жанру Action протягом років?
    grouped_rating_df = ratings_df.join(title_basics_df, "tconst").filter("runtimeMinutes >= 50 AND genres = 'Action'").groupBy("startYear").agg({"averageRating": "avg"}).orderBy("startYear")
    ...
    decade_df = df_movies.withColumn("decade", f.floor(df_movies["startYear"] / 10) * 10).filter("averageRating >= 8").groupBy("decade", "genres").agg({"runtimeMinutes": "avg"}).orderBy("decade")
    ...
    ratings_genre_df = joined_df.withColumn("cumulative_ratings", sum("numVotes").over(window_spec)).orderBy("cumulative_ratings", ascending=False)

    ratings_genre_df.drop('startYear').drop('genres').drop('endYear').drop('runtimeMinutes').show()

    # Як змінювався рейтинг фільмів у жанрах 'Action', 'History', 'Crime', 'Sci-Fi' протягом років?
    avg_rating_per_year_genre = ratings_df.join(title_basics_df, "tconst").filter("genres IN ('Action', 'History', 'Crime', 'Sci-Fi')").groupBy("startYear", "genres").agg({"averageRating": "avg"}).orderBy('startYear')

    avg_rating_per_year_genre.show()

    # Скільки фільмів виходили кожного року і яка кількість відгуків та середня оцінка?
    movies_per_year = ratings_df.join(title_basics_df, "tconst").groupBy("startYear").agg({"averageRating": "avg", "tconst": "count", "numVotes": "sum"}).orderBy("startYear")
