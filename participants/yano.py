...
def load_name_basics_df(path, spark_session, f):
    ...
    """
        ...
        Отож, прийнято рішення видалити колонки (birth_year, death_year) через надто малу к-сть not null значень і не здатність заповнити пропущені значення
...
def load_title_akas_df(path, spark_session, f):
    if not os.path.isfile(paths.PATH_LOCALES_CUSTOM):
        print(f"Parsing site to get extra data for 'title.akas' ... ")
